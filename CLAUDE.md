...
```text
Release ONDEWO Survey Nodejs Client ${ONDEWO_SURVEY_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO Survey Nodejs Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO Survey Nodejs Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/survey-client-nodejs`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/survey-client-nodejs versions --json
```
